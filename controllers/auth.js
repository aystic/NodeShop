...
exports.postReset = (req, res, next) => {
  const email = req.body.email;
  if (["user1@gmail.com", "user2@gmail.com"].includes(email)) {
    req.flash(
      "error",
      "This account is protected. The password for this user cannot be changed!"
    );
...
exports.getUpdate = (req, res, next) => {
  ...
  User.findOne({
    resetToken: token,
    resetTokenExpiry: { $gt: new Date().getTime() },
  })
    .then((user) => {
      if (user) {
